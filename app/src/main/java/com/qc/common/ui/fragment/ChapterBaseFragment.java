 ...
 * @ver 1.0
 */
public class ChapterBaseFragment extends BaseTabFragment implements ChapterView {
    ...
    public static ChapterBaseFragment getInstance(Entity entity) {
        ChapterBaseFragment fragment = new ChapterBaseFragment();
    ...
    public Animation onCreateAnimation(int transit, boolean enter, int nextAnim) {
        if (enter && entity != null && !fragments.isEmpty()) {
            ((TstFragment) fragments.get(INDEX)).updateData();
            setValue();
    ...
    private void setListener() {
        ...
        tvUpdateChapter.setOnClickListener(v -> {
            if (checkNotEmpty()) {
                ...
                ((TstFragment) fragments.get(INDEX)).start();
            } else {
                showFailTips("暂无" + TmpData.content + "章节");
            }
        });
        ...
        tvRead.setOnClickListener(v -> {
            if (checkNotEmpty()) {
                ((TstFragment) fragments.get(INDEX)).startRead();
            } else {
                showFailTips("暂无" + TmpData.content + "章节");
            }
        });
    ...
    protected void addFragment(ArrayList<BaseFragment> fragments) {
        for (String key : entity.getInfo().getChapterInfoMap().keySet()) {
            fragments.add(TstFragment.getInstance(key, entity));
        }
    }

    @Override
    protected int getContentViewId() {
        return R.layout.fragment_chapter_base;
    }
}
